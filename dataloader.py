...
class NextSentenceDataloader(Dataset):
    ...
    def __build_sentense(self):
        while True:
            ...
            self.__set_start_and_end(file_path)

    def __set_start_and_end(self, file_path):
        with open(file_path, "r", encoding="utf-8") as f:
            li =  f.readlines()
            
            for idx in range(1, len(li)-5):
                source = '文章と文章の間の文を補完します。' + '始まり: ' + li[idx] + '\n' + '終わり: ' + li[idx+3] + self.__sep + li[idx] + li[idx+1] + li[idx+2] + li[idx+3] 
                # source = source.strip()
